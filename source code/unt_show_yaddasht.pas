...
begin
    if DataModule_gozaresh_tarkibi.ADOQuery_yaddasht.RecordCount<=0 then
      MessageBox(Handle,'Ì«œœ«‘ Ì  »—«Ì «‰Ã«„ ÊÃÊœ ‰œ«—œ',' ÊÃÂ !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING )
    else
    begin
    if DataModule_gozaresh_tarkibi.ADOQuery_yaddashtanjam.AsString<>'«‰Ã«„ ‰‘œ'then
         MessageBox(Handle,'Ì«œœ«‘  «‰ Œ«»Ì ﬁ»·¬ «‰Ã«„ ‘œÂ «” ',' ÊÃÂ !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING )
    else
    begin
        s:=pchar('«‰Ã«„ Ì«œœ«‘  —«  «ÌÌœ „Ì‰„«ÌÌœ ø' );
        if MessageBox(Handle,s,' «ÌÌœ !',MB_YESNO or MB_ICONQUESTION or MB_RIGHT or MB_RTLREADING )=id_yes then
          begin
            ...
            begin
              frm_main.l_last_op.Caption:=' «‰Ã«„ Ì«œœ«‘  „—»Êÿ »Â '+ DataModule_gozaresh_tarkibi.ADOQuery_yaddashtshakhs.AsString +
            ...
            begin
              frm_main.l_last_op.Caption:=' «‰Ã«„ Ì«œœ«‘  »« ⁄‰Ê«‰ '+ DataModule_gozaresh_tarkibi.ADOQuery_yaddashtmozo.AsString +
              ' À»  ‘œÂ œ—  «—ÌŒ '+DataModule_gozaresh_tarkibi.ADOQuery_yaddashttarikh_zakhire.AsString;
              frm_main.sabte_amaliat(frm_main.l_last_op.Caption+' Ê  «—ÌŒ —”ÌœêÌ '+DataModule_gozaresh_tarkibi.ADOQuery_yaddashttarikh_yadavary.AsString+
              ' »« „ ‰ '+DataModule_gozaresh_tarkibi.ADOQuery_yaddashtmatn.AsVariant);
            end;
              ...
              DataModule_gozaresh_tarkibi.ADOQuery_yaddasht.SQL.Text:='update T_yaddasht set anjam='+QuotedStr('«‰Ã«„ ‘œ')+' where id='+inttostr(id_);
              DataModule_gozaresh_tarkibi.ADOQuery_yaddasht.ExecSQL;


            MessageBox(Handle,'Ì«œœ«‘  «‰ Œ«»Ì «‰Ã«„ ‘œ',' ÊÃÂ !',MB_OK or MB_ICONINFORMATION or MB_RIGHT or MB_RTLREADING );
          end
        else
           MessageBox(Handle,'Ì«œœ«‘  «‰ Œ«»Ì «‰Ã«„ ‰‘œ',' ÊÃÂ !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING );
    end;    
    end;
