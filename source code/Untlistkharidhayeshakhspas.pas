...
begin
  if (DataModule1.ADOQuery3shahrforoshande.AsString='') then
     MessageBox(Handle,'‰«„ Ê‘Â— —« «‰ Œ«» ‰„«ÌÌœ','Œÿ«! ',MB_OK or MB_ICONEXCLAMATION or MB_RTLREADING or MB_RIGHT )
  else
  begin
      showhesabshakhs;
      frm_factor.type_.Caption:='Â„Â ”«·Â«ÌÌ òÂ Œ—Ìœ ‰„ÊœÂ';
      frm_factor.QRLabel_type2.Caption:= 'Â„Â ”«·Â«ÌÌ òÂ Œ—Ìœ ‰„ÊœÂ';
  end;
end;
...
begin
  ...
  if DataModule1.ADOQuery3.RecordCount > 0 then
   begin
    gotofactor_kolsalha ;
    bede_talab_kolsalha;
    frm_factor.type_.Caption:='œ— ò· ”«·Â«Ì Œ—Ìœ';
    frm_factor.QRLabel_type2.Caption:= 'œ— ò· ”«·Â«Ì Œ—Ìœ';
    sleep(1000);
  ...
  if DataModule1.ADOQuery3.RecordCount > 0 then
   begin
    bede_talabdarsal;
    showhesabdarsal;
    frm_factor.type_.Caption:=' œ— ”«·'+' '+scsal.Text;
    frm_factor.QRLabel_type2.Caption:=' œ— ”«·'+' '+scsal.Text;
    sleep(1000);
    frm_factor.QuickRep1.Preview;
   end
    else
...
begin
  if (scsal2.Text='') or (scmahall.Text='') then
    MessageBox(Handle,'”«· Ê „«Â —« «‰ Œ«» ‰„«ÌÌœ',' ÊÃÂ !',MB_OK or MB_ICONEXCLAMATION or MB_RTLREADING or MB_RIGHT)
  else
  begin
    if DataModule1.ADOQuery3.RecordCount > 0 then
    begin
...
begin
    ...
    frm_factor.type_.Caption:='œ— ”«·'+scsal2.Text+' „«Â '+' '+scmahall.Text;
    frm_factor.QRLabel_type2.Caption:='œ— ”«·'+scsal2.Text+' „«Â '+' '+scmahall.Text;
    sleep(1000);
    frm_factor.QuickRep1.Preview;
end
else
