...
begin
        ...
        tarikh:=Edit6.Text;
        if type_='a' then
          amaliat:=' Å—œ«Œ  »œÂÌ »Â '+DataM_final.ADOQ_bedeh_be_foroshnseshahrforoshande.AsString+' »Â „»·€ '+Edit9.Text+' '+'„»·€ << '+FloatToStr(t_j)+' >> «“ ÿ·» ‘„«'+' Ê „»·€ << '+FloatToStr(b_j)+' >> «“ »œÂÌ ‘„« ò„ ‘œ. ';
        if type_='b' then
          amaliat:=' Å—œ«Œ  »œÂÌ »Â '+DataM_final.ADOQ_bedeh_be_foroshnseshahrforoshande.AsString+' »Â „»·€ '+Edit9.Text+' '+' „»·€ << '+FloatToStr(b_j)+' >> «“ »œÂÌ ‘„« ò„ ‘œ. ' ;
        if type_='c' then
          amaliat:=' Å—œ«Œ  »œÂÌ »Â '+DataM_final.ADOQ_bedeh_be_foroshnseshahrforoshande.AsString+' »Â „»·€ '+Edit9.Text+' '+' „»·€ << '+FloatToStr(b_j)+' >> «“ »œÂÌ ‘„« ò„ ‘œ. ' ;
          ...
          DataM_sp.ADOStoredProc_pardakhte_bedehi_be_foroshande.Parameters[8].Value:='Å—œ«Œ  ‰ﬁœÌ »œÂÌ »Â ›—Ê‘‰œÂ'+DataM_final.ADOQ_bedeh_be_foroshnseshahrforoshande.AsString;//@tozih nvarchar(150),
          DataM_sp.ADOStoredProc_pardakhte_bedehi_be_foroshande.Parameters[9].Value:='Œ—ÊÃÌ';//@type nvarchar(6),
          ...
          frm_main.l_last_op.Caption:=' Å—œ«Œ  »œÂÌ »Â '+DataM_final.ADOQ_bedeh_be_foroshnseshahrforoshande.AsString;
...
begin
  ...
  begin
    max_bedehi:=false;
    mb:=pchar('„»·€ Ê«—œ ‘œÂ «“ ò· ÿ·» ‘Œ’ = '+FloatToStr(bedeh)+' »Ì‘ — „Ì»«‘œ. ');
...
begin
  if talab >0 then
  begin
    st:=pchar('‘„« «“  <<  '+Lname.Caption+' >> „»·€ << '+FloatToStr(talab)+' >> ÿ·»ò«— „Ì»«‘Ìœ° „»·€ Å—œ«Œ Ì «“ „»·€ ÿ·» ‘„« ò„ ‘Êœø(œ— ’Ê— Ì òÂ „»·€ Å—œ«Œ Ì ‘„« «“ „»·€ ÿ·» ‘„« »Ì‘ — »«‘œ ÿ·» ‘„« «“ ›—Ê‘‰œÂ ’›— ‘œÂ Ê „«»ﬁÌÂ Å—œ«Œ  «“ »œÂÌ ‘„« ò„ „Ì ‘Êœ).');
    ...
    begin
       ...
       {DataM_final.ADOQ_bedeh_be_foroshnse2.SQL.Text:='update T_bede_kol_be_foroshande set bedeh=bedeh-'+FloatToStr(b_j)+
        ...
        frm_main.l_last_op.Caption:=' Å—œ«Œ  »œÂÌ »Â '+Lname.Caption;
        frm_main.sabte_amaliat(' Å—œ«Œ  »œÂÌ »Â '+Lname.Caption+' »Â „»·€ '+Edit9.Text+' '+'„»·€ << '+FloatToStr(t_j)+' >> «“ ÿ·» ‘„«'+' Ê „»·€ << '+FloatToStr(b_j)+' >> «“ »œÂÌ ‘„« ò„ ‘œ. ');

        st:=pchar('„»·€ << '+FloatToStr(t_j)+' >> «“ ÿ·» ‘„«'+' Ê „»·€ << '+FloatToStr(b_j)+' >> «“ »œÂÌ ‘„« »Â << '+Lname.Caption+' >> ò„ ‘œ. ' );
    ...
    begin
        ...
        frm_main.l_last_op.Caption:=' Å—œ«Œ  »œÂÌ »Â '+Lname.Caption;
        frm_main.sabte_amaliat(' Å—œ«Œ  »œÂÌ »Â '+Lname.Caption+' »Â „»·€ '+Edit9.Text+' '+' „»·€ << '+FloatToStr(b_j)+' >> «“ »œÂÌ ‘„« ò„ ‘œ. ' );

        st:=pchar(' „»·€ << '+FloatToStr(b_j)+' >> «“ »œÂÌ ‘„« »Â << '+Lname.Caption+' >> ò„ ‘œ. ' );
        MessageBox(Handle,st,' ÊÃÂ!',MB_OK + MB_ICONINFORMATION or MB_RTLREADING or MB_RIGHT);}
...
if trim(Edit9.Text)='' then
  ...
  begin
    MessageBox(Handle,'‘Œ’ °  «—ÌŒ Å—œ«Œ  Ê „»·€ Å—œ«Œ Ì  ”«⁄  Å—œ«Œ  »«Ìœ Ê«—œ ‘Ê.œ','Œÿ« !',MB_OK or MB_ICONEXCLAMATION or MB_RTLREADING or MB_RIGHT);
    Exit;
  end
  else
 begin
  if (DBLookupComboBox3.Text='') then
   ...
   mojodi:=Frm_gozaresh_az_sandog.mojodi_kol(DBLookupComboBox3.Text);
    ...
    if Frm_shakhs_jadid.check_exsist_shakhs(Lname.Caption) then
      ...
      begin
        st:=pchar('Å—œ«Œ  »œÂÌ »Â << '+Lname.Caption+' >> —«  «ÌÌœ „Ì‰„«ÌÌœø');
        if   MessageBox(Handle,st,' ÊÃÂ !',MB_YESNO or MB_ICONQUESTION or MB_RIGHT or MB_RTLREADING )=id_yes then
        begin
