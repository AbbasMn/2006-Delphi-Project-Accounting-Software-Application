...
begin
 DataM_final.ADOQ_t_Temp_pardakht_check_forosh.SQL.Text:=' begin tran insert into t_Temp_pardakht_check_forosh(sh_hesab,tarikh'+
  ...
  trim(Edit_mablagh.Text)+','+QuotedStr(DataM_final.ADOQ_t_sh_facrorhaye_foroshshahrforoshande.AsString)+','+QuotedStr(DataM_final.ADOQ_t_sh_facrorhaye_foroshtarikh.AsString)+','+QuotedStr('ŒÌ—')+','+
...
begin
  ...
  begin
      ...
      MessageBox(Handle,'çò »« ‘„«—Â Õ”«» Ê ‘„«—Â çò Ê«—œ ‘œÂ °»—«Ì ›—Ê‘Â«Ì „«ﬁ»· À»  ‘œÂ «” ',' ÊÃÂ !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING );
  end
  /////////////// search temp check //////////////
  else
  begin
    ...
    begin
      check_keshide_shodan_ghabl:=true;
      MessageBox(Handle,'çò »« ‘„«—Â Õ”«» Ê ‘„«—Â çò Ê«—œ ‘œÂ ° »—«Ì Â„Ì‰ ‘„«—Â ›—Ê‘ Ê«—œ ‘œÂ «” ',' ÊÃÂ !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING );
    end;
  end;
...
begin
 ...
 DataM_final.ADOQ_t_Temp_pardakht_check_forosh.SQL.Text:='begin tran insert into t_Temp_pardakht_check_forosh(sh_hesab,tarikh'+
  ...
  QuotedStr(trim(dbanbar.Text)+' '+trim(E_shobe.Text)+' '+trim(E_shomare_hesab.Text))+','+QuotedStr(Edit_tarikh.Text)+','+
  QuotedStr(trim(Edit_shomare.Text))+','+
  trim(Edit_mablagh.Text)+','+QuotedStr(DataModule1.ADOQuery3shahrforoshande.AsString)+','+QuotedStr(frm_forosh_property.tarikh)+','+QuotedStr('ŒÌ—')+','+
  code+','+s+')'+' commit tran';
...
begin
  ...
  begin
    ...
    if (L_type.Caption='forosh') then
     ...
     if (frm_forosh_fishi.Caption<>'«Ã‰«” «‰ Œ«» ‰‘œÂ «‰œ')then
      begin
        if not check_keshide_shodan_ghabl then
        begin
          insert_check_to_temp_check_forosh;
