...
begin
  if DataM_final.ADOQ_T_pardakht_check_forosh.RecordCount <=0 then
    MessageBox(Handle,'çòÌ »—«Ì Å«” ‘œ‰ ÊÃÊœ ‰œ«—œ',' ÊÃÂ !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING)
  else
    begin
      frm_main.l_last_op.Caption:=' Å«” ò—œ‰ çò »Â ‘„«—Â '+DataM_final.ADOQ_T_pardakht_check_foroshsh_check.AsString+' œ—Ì«›  ‘œÂ «“ '+DataM_final.ADOQ_T_pardakht_check_foroshshahrforoshande.AsString;
      ...
      DataM_sp.ADOStoredProc_pas_kardane_check.Parameters[3].Value:='»·Ì';//@pass  nvarchar(10),
      DataM_sp.ADOStoredProc_pas_kardane_check.Parameters[4].Value:=frm_main.l_last_op.Caption+' »Â „»·€ '+DataM_final.ADOQ_T_pardakht_check_foroshmablagh_check.AsString;//@amaliat nvarchar(1000)
...
procedure TFrm_paskardancheck.showcheck;
  begin
    ...
    DataM_final.ADOQ_T_pardakht_check_forosh.sql.Text:='select * from T_pardakht_check_forosh where pass='+QuotedStr('ŒÌ—')+
...
begin
  ...
  begin
    MessageBox(Handle,'’‰œÊﬁÌ »—«Ì Ê—Êœ „»·€ çò Å«” ‘œÂ »Â ¬‰° «‰ Œ«» ‰‘œÂ «” ','Œÿ« !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING);
    exit;
  end;

  if (RadioGroup2.ItemIndex=1)and(Dmlistha.ADOQuery_shomare_hesabe_banki.RecordCount=0)then
  begin
    MessageBox(Handle,'‘„«—Â Õ”«» »«‰òÌ »—«Ì Ê—Êœ „»·€ çò Å«” ‘œÂ »Â ¬‰° «‰ Œ«» ‰‘œÂ «” ','Œÿ« !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING);
    exit;
  end;
  ...
  begin
    MessageBox(Handle,'‘„«—Â ›Ì‘ —« Ê«—œ ‰„«ÌÌœ','Œÿ« !',MB_OK or MB_ICONEXCLAMATION or MB_RIGHT or MB_RTLREADING);
    exit;
  end;
  ...
  s:='Å«” ò—œ‰ Ê Ê«—Ì“ çò «‰ Œ«»Ì »Â ';

  if RadioGroup2.ItemIndex=0 then
    s:=s+' ’‰œÊﬁ << '+DBLookupComboBox1.Text+' >>  —«  «ÌÌœ „Ì‰„«ÌÌœø'
  else
    s:=s+' ‘„«—Â Õ”«» »«‰òÌ << '+Dmlistha.ADOQuery_shomare_hesabe_bankibank_shobe_sh_hesab.AsString+' >>  —«  «ÌÌœ „Ì‰„«ÌÌœø';
  ...
  begin
      ...
      begin
        ...
        frm_add_bank1.sharh:='Å«” ò—œ‰ çò »« ‘„«—Â Õ”«» '+DataM_final.ADOQ_T_pardakht_check_foroshsh_hesab.AsString+' Ê ‘„«—Â çò '+DataM_final.ADOQ_T_pardakht_check_foroshsh_check.AsString+
        ' ê—› Â ‘œÂ «“ '+DataM_final.ADOQ_T_pardakht_check_foroshshahrforoshande.AsString+' ‘„«—Â Œ—Ìœ '+DataM_final.ADOQ_T_pardakht_check_foroshshomare_factor.AsString;
      ...
      st:=pchar(' çò »«‘„«—Â << '+DataM_final.ADOQ_T_pardakht_check_foroshsh_check.AsString+' >>œ—Ì«›  ‘œÂ «“ << '+DataM_final.ADOQ_T_pardakht_check_foroshshahrforoshande.AsString+' >> Å«” ‘œ');
      ...
      begin
        ...
        QuotedStr('Å«” ò—œ‰ çò »« ‘„«—Â Õ”«» '+DataM_final.ADOQ_T_pardakht_check_foroshsh_hesab.AsString+' Ê ‘„«—Â çò '+DataM_final.ADOQ_T_pardakht_check_foroshsh_check.AsString+
        ' ê—› Â ‘œÂ «“ '+DataM_final.ADOQ_T_pardakht_check_foroshshahrforoshande.AsString+' ‘„«—Â Œ—Ìœ '+DataM_final.ADOQ_T_pardakht_check_foroshshomare_factor.AsString)+
        ','+QuotedStr('Ê—ÊœÌ')+
